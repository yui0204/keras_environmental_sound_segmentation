...
def build_pspnet(nb_classes, resnet_layers, input_shape, activation='softmax', 
                 n_channels=1, mask=False, trainable=False):
    """Build PSPNet."""
    ...
    inp = Input((input_shape[0], input_shape[1], n_channels))
    ...
    if mask == True:
        ...
        # GLU
        a = Conv2D(nb_classes, (3, 3), padding='same', dilation_rate=1)(x)
        a = BatchNormalization()(a)   
        g = Conv2D(nb_classes, (3, 3), activation='sigmoid', 
                   padding='same', dilation_rate=1)(x)
        g = BatchNormalization()(g)
        x = multiply([a, g])
        
        x = concatenate([x, inp], axis=-1)
    else:
        x = inp
        
    res = ResNet(x, layers=resnet_layers)
