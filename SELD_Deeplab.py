...
def xception_block(inputs, depth_list, prefix, skip_connection_type, stride,
    ...
    residual = inputs
    for i in range(3):
        residual = SepConv_BN(residual, depth_list[i],
...
def Deeplabv3(weights='None', input_tensor=None, 
    ...
    This model is available for TensorFlow only, and can only be used with 
    inputs following the TensorFlow data format `(width, height, channels)`.
    # Arguments
...
#    x = BilinearUpsampling(output_size=(input_shape[0], input_shape[1]))(x)
    x = BilinearUpsampling(output_size=(8, input_shape[1]))(x)
